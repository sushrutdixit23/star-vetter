...
def run_stage(name, script_name, project_root, log, logf):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        # Stream the child script's own output straight through to both the
        # console and the log file, line by line, as it runs - this is what
        # makes an unattended run reviewable afterwards: the log file ends up
        # holding the exact same transcript you'd see running each script by
        # hand, plus the orchestrator's own decisions around it.
...
def main():
    ...
    with open(log_path, "w", encoding="utf-8", newline="\n") as logf:
        ...
        if not run_stage("F2c fetch light curves", "f2c_fetch_lightcurves.py", project_root, log, logf):
            sys.exit(1)
        ...
        # ---- F3i: BLS vetting ----
        if not run_stage("F3i vet light curves", "f3i_vet.py", project_root, log, logf):
            sys.exit(1)
        ...
        if not run_stage("F4f novelty cross-match (pass 1 of 2)", "f4f_novelty.py",
                          project_root, log, logf):
            sys.exit(1)
        ...
        if not run_stage("Neighbour contamination check", "diag_blend.py", project_root, log, logf):
            sys.exit(1)
        ...
        if not run_stage("F4f novelty cross-match (pass 2 of 2, applies contamination)",
                          "f4f_novelty.py", project_root, log, logf):
            sys.exit(1)
        ...
        if not run_stage("Pixel-level source check", "diag_pixel.py", project_root, log, logf):
            sys.exit(1)
        ...
        if not run_stage("F5 build dossiers", "f5_dossier.py", project_root, log, logf):
            sys.exit(1)
